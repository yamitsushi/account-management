<template>
    <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion"  v-bind:class="hideSidebar()">
        ...
        <li class="nav-item" v-bind:class="isActive('account')" v-if="can(['USER.READ', 'ROLE.READ'])">
            ...
            <b-collapse id="collapseTwo">
                <div class="bg-white py-2 collapse-inner rounded">
                    ...
                    <router-link class="collapse-item" :to="{ name : 'account.user'}" v-if="can('USER.READ')" exact-path>
                        <i class="fas fa-fw fa-users"></i>
                        <span>User Controller</span>
                    </router-link>
                    <router-link class="collapse-item" :to="{ name : 'account.role'}" v-if="can('ROLE.READ')" exact-path>
